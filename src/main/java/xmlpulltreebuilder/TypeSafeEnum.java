...
/**
 * Yet another type safe enumeration base class. This class supports the creation of multiple enumeration subclasses. In each subclass the enumeration values
 * start at 0.
 * 
 * <p>
 * Documentation for this class can be found <a href="http://www.bearcave.com/software/java/misl/enum/type_safe_enum.html"> here</a>. Apparently TypeSafeEnums
 * are supported in the new version of Java. So at some point the classes that use this class should be replaced with something more generic.
 * </p>
 */
public abstract class TypeSafeEnum {
	...
	@SuppressWarnings("rawtypes")
	public static Iterator<TypeSafeEnum> enumValues(Class cls) {
		Iterator<TypeSafeEnum> iter = null;
	...
	/**
	 * Find the entry for the enumeration, if it exists. If not, add it to the end of the enumInfo. Note that this function has linear time, but the assumption
	 * is that there will not a large number of enumeration classes.
	 */
	@SuppressWarnings("rawtypes")
	private static enumInfo findInfo(Class cls, boolean add) {
		enumInfo foundElem = null;
		int hashCode = cls.hashCode();
		for (enumInfo elem : infoVec) {
