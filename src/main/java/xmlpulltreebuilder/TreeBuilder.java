 ...
 *
 * <p>
 * This class supports the construction of a light weight in memory tree from an XML file. Like the W3C DOM Document object, the structure of this tree reflects
 * the structure of the XML.
 * </p>
 * <p>
 * The tree that is constructed is designed for rapid traversal and in memory modification. It also has the advantage of using less memory than the java.sun.com
 ...
 * </p>
 * <p>
 * A few notes about attributes and namespaces. In general I think that the XmlPullParser rocks. The API is will designed and the calls mostly make sense.
 * But... the XmlPullParser does not treat all attributes the same way. In particular it does not treat name space definitions like other attributes. This can
 * be seen in XML designed to be processed via a schema. For example:
 * </p>
 *
 * <pre>
 *           &lt;ex:EXPRESSION
 ...
 * </pre>
 * <p>
 * Here there are two name space definitions. One defining the namespace associated with the "ex" prefix and one associated with XML schemas (for the schema
 * location attribute).
 * </p>
 * <p>
 * The getAttributeCount() method will return 1 when processing the EXPRESSION tag. This is because the name space definitions (the attributes with the xmlns
 * prefix) are not treated as normal attributes. And sometimes this is good, because these attributes are not necessarily of interest. In this case, however,
 * the intent is to exactly mirror the XML in an in-memory tree. So that if the tree is serialized the original XML will be recovered (with the exception of
 * white space TEXT, since this is not included).
 * </p>
 * <p>
 * The attributes are only available when the END_TAG element for the document tag is processed. The getDepth() method tells the current XML nesting depth, so
 * it can be determined that an END_TAG is the document end tag. The attributes are then fetched and prepended to the attribute list. Sort of awkward. This is
 * the one place where I would differ in the design of the XmlPullParser. I'd treat all attributes the same way, including those with the "xmlns" prefix. Then
 * the user can simply ignore operands with the namespace prefix.
 * </p>
 *
 * @author Ian Kaplan, www.bearcave.com, iank@bearcave.com
 */
public class TreeBuilder {
	...
	/**
	 * This method is called for the end tag of the root document tag. If there are name spaces, they would have been defined in this tag. Insert them in the
	...
	/**
	 * Build a tag node. Note that a tag node always has an AttributeList object, even if there is no attribute list.
	 */
	...
	private TreeNode buildTree() throws XmlPullParserException, IOException {
		...
		do {
			...
			if (event == XmlPullParser.START_TAG) {
				...
				String text = mParser.getText();
				root = new TextNode(TreeNodeType.CDATA, text);
				done = true;
	...
	public TagNode parse(Reader reader) throws XmlPullParserException, IOException {
		TreeNode root = null;
		mParser = getParser();
		if (mParser != null) {
	...
	public TagNode parse(String xml) throws XmlPullParserException, IOException {
